...
public class PizzaIRBuilder {
    ...
    private void registerBuiltinClass() {
        ...
        Arrays.asList("__lgcnot__").forEach(method -> {
            Func func = new Func(new FuncAddr().addClass(c_bool).add(method), method, t_bool, new ParamList());
            c_bool.addMethod(func);
            ir.funcList.addFunc(func);
        });
        Arrays.asList("__lgcand__", "__lgcor__", "__eq__", "__ne__").forEach(method -> {
            Func func = new Func(new FuncAddr().addClass(c_bool).add(method), method, t_bool,
        ...
        Arrays.asList("__pos__", "__neg__", "__bitinv__", "__preinc__", "__predec__", "__postinc__", "__postdec__")
                .forEach(method -> {
                    Func func = new Func(new FuncAddr().addClass(c_int).add(method), method, t_int, new ParamList());
        ...
        Arrays.asList("__add__", "__sub__", "__mul__", "__div__", "__mod__", "__shl__", "__shr__", "__bitand__",
                "__bitxor__", "__bitor__").forEach(method -> {
                    Func func = new Func(new FuncAddr().addClass(c_int).add(method), method, t_int,
                            new ParamList(Arrays.asList(t_int)));
                    c_int.addMethod(func);
                    ir.funcList.addFunc(func);
                });
        Arrays.asList("__lt__", "__gt__", "__le__", "__ge__", "__eq__", "__ne__").forEach(method -> {
            Func func = new Func(new FuncAddr().addClass(c_int).add(method), method, t_bool,
                    new ParamList(Arrays.asList(t_int)));
            c_int.addMethod(func);
            ir.funcList.addFunc(func);
        });
        ...
        Arrays.asList("__add__").forEach(method -> {
            Func func = new Func(new FuncAddr().addClass(c_string).add(method), method, t_string,
                    new ParamList(Arrays.asList(t_string)));
            c_string.addMethod(func);
            ir.funcList.addFunc(func);
        });
        Arrays.asList("__lt__", "__gt__", "__le__", "__ge__", "__eq__", "__ne__").forEach(method -> {
            Func func = new Func(new FuncAddr().addClass(c_string).add(method), method, t_bool,
                    new ParamList(Arrays.asList(t_string)));
            c_string.addMethod(func);
            ir.funcList.addFunc(func);
        });
        Arrays.asList("length", "parseInt").forEach(method -> {
            Func func = new Func(new FuncAddr().addClass(c_string).add(method), method, t_int, new ParamList());
            c_string.addMethod(func);
            ir.funcList.addFunc(func);
        });
        Arrays.asList("ord").forEach(method -> {
            Func func = new Func(new FuncAddr().addClass(c_string).add(method), method, t_int,
                    new ParamList(Arrays.asList(t_int)));
            c_string.addMethod(func);
            ir.funcList.addFunc(func);
        });
        ...
        Arrays.asList("size").forEach(method -> {
            Func func = new Func(new FuncAddr().addClass(c_array).add(method), method, t_int, new ParamList());
    ...
    private void registerBuiltinFunc() {
        Func __init__ = new Func(new FuncAddr().add("__init__"), "__init__", ir.typeTable.get("void"), new ParamList());
        ir.funcList.addFunc(__init__);

        Func print = new Func(new FuncAddr().add("print"), "print", ir.typeTable.get("int"),
                new ParamList(ir.typeTable.get("string")));
        ir.funcList.addFunc(print);

        Func println = new Func(new FuncAddr().add("println"), "println", ir.typeTable.get("void"),
                new ParamList(ir.typeTable.get("string")));
        ir.funcList.addFunc(println);

        Func getInt = new Func(new FuncAddr().add("getInt"), "getInt", ir.typeTable.get("int"), new ParamList());
        ir.funcList.addFunc(getInt);

        Func getString = new Func(new FuncAddr().add("getString"), "getString", ir.typeTable.get("string"),
                new ParamList());
        ir.funcList.addFunc(getString);

        Func toString = new Func(new FuncAddr().add("toString"), "toString", ir.typeTable.get("string"),
